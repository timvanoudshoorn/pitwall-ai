...
- **Car classes:** F1 2025 season (base game, 10 teams + Konnersport as fictional 11th), **F1® 25: 2026 Season Pack** (new regs — Overtake Mode, Active Aerodynamics, different fuel/ERS behavior), F2 (2024 and 2026 chassis), Braking Point's Konnersport, APXGP (the fictional team from the *F1* movie, Iconic Edition content), Icons/legends cars (Driver Career now lets AI recruit legends like Schumacher or Senna into current-spec cars), and the standard **F1 World** car (the generic/shared car used in F1 World mode, distinct from any specific team's real-world car).
...
- **Tracks:** the full real-world calendar plus reverse layouts, with special handling for the LiDAR-scanned venues (Silverstone, Bahrain, Imola, Melbourne, Miami, Suzuka, Red Bull Ring, Zandvoort) since their surface/kerb data is the most accurate in the game.
